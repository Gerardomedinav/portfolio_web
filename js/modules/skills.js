...
import { getSkillsData } from './dataStore.js';
...
export function renderSkillsCategory(skillsArray, containerId, lang) {
  ...
  skillsArray.forEach(skill => {
    ...
    const altText = (skill.imageAlt && (skill.imageAlt[lang] || skill.imageAlt.es)) ||
      (lang === 'es'
        ? `Logo de ${skill.name}, tecnología utilizada por Gerardo Medina`
        : `Logo of ${skill.name}, technology used by Gerardo Medina`);
...
export function initSkills() {
  ...
  document.addEventListener('skillsDataChange', () => {
    loadSkills();
  });
}
